...
FROM ageekinside/base as builder
...
RUN wget --no-check-certificate "https://github.com/ninja-build/ninja/releases/download/v1.8.2/ninja-linux.zip" && \
...
RUN mkdir /tmp/cmake-install && cd /tmp/cmake-install && \
    wget --no-check-certificate "https://cmake.org/files/v3.7/cmake-3.7.2-SHA-256.txt.asc" && \
    wget --no-check-certificate "https://cmake.org/files/v3.7/cmake-3.7.2-SHA-256.txt" && \
